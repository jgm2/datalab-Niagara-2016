	...
	protected void processTuple(Tuple inputTuple, int streamId)
		...
		if (predEval.evaluate(inputTuple, null)) {
			...
			if (exploit) {
				...
				if (!guardMatch) {
					/*String value = ((BaseAttr) inputTuple.getAttribute(0)).attrVal().toString();
					int dummyValue=Integer.parseInt(value);
					if(-1!=dummyValue){*/
				...
				/*//sleep for time on sleep attribute
				int sleepAttrSchemaPosition = inputTupleSchemas[0].getPosition("sleep");
				String sleepvalue = ((BaseAttr) inputTuple.getAttribute(sleepAttrSchemaPosition)).attrVal().toString();
				int sleepTime = Integer.parseInt(sleepvalue);
				if(0!=sleepTime){
