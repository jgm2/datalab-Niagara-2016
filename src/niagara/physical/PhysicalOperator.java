...
/**
 * This is the Abstract <code>PhysicalOperator</code> class from which all
 * operator are derived. It provides the basic control flow and all each
 * operator has to do is to implement stubs.
 * 
 * @version 1.0
 */

@SuppressWarnings("unchecked")
public abstract class PhysicalOperator extends PhysicalOp implements
		SchemaProducer, SerializableToXML, Initializable, Schedulable,
		Instrumentable {

	/*
	 * The lifecycle of a physical operator can be divided in three phases:
	 * 
	 * 1. First, all physical ops have a no argument constructor, and so they
	 * may exist without any initialization whatsoever. This is mainly for use
	 * in rule patterns, where they just stand for representatives of their
	 * class.
	 * 
	 ...
	 * Optimization is the only time where copy() is called - that's why it
	 * doesn't care about the run time data members of the operator. Many copies
	 * of a physical operator may be around, and most of them will never be
	 * executed, so it makes no sense to initialize things that are just used
	 * for operator execution at this point.
	 * 
	 * 3. After the optimization is complete, the scheduler initializes the
	 * physical operator with opInitialize() (called by initialize()). If a
	 * physical operator survived this far, it's going to be executed, so
	 * opInitialize() does the rest of the required initializations (and also if
	 * there's anything that doesn't need to be there for optimization, like
	 * hashtable allocations etc. is done in opInitialize()).
	 */
	...
	private boolean debug = false;  // (JGM)
	 ...
	 * (b) the stream from which the result was read.
	 */
	...
	/**
	 * Initialize the PhysicalOperator with the appropriate source streams, sink
	 * streams, whether the operator is blocking or non-blocking, the data
	 * manager it is working with, and the responsiveness of the operator to
	 * control information.
	 * 
	 * @param sourceStreams
	 ...
	 * @param blockingSourcetreams
	 *            A boolean array that specifies the source streams due to which
	 *            the operator blocks.
	 * @param responsiveness
	 *            The responsiveness, in milliseconds, to control messages
	 */
	...
	/**
	 * Is this operator ready for execution (All sink streams in place?)
	 */
	...
	/**
	 * This function sets up the flow of control for the operator by reading
	 * from input streams and writing to sink streams. All control messages are
	 * handled here.
	 * 
	 */
...
//		if(this instanceof niagara.physical.PhysicalHashJoin)
				...
				if (!(sourceObject.tuple == null)) {
					...
					if (sourceObject.tuple.isPunctuation())
					{
						...
						if (isBlocking()) {
							...
							processTuple(sourceObject.tuple,
									sourceObject.streamId);
						}
					}
				} else {
	...
	public void addSinkStream(SinkTupleStream newStream) {
		...
		for (idx = 0; idx < sinkStreams.length; idx++)
			if (sinkStreams[idx] == null)
				break;
		assert idx < sinkStreams.length : "Attempt to add output stream to an "
				+ getClass() + " operator that's already full" + "(" + idx
				+ "/" + sinkStreams.length + ")";
		sinkStreams[idx] = newStream;
	}

	/**
	 * add this stream to sink streams at index i - boy, I do not like this
	 * function or its use in ExecutionScheduler, but I don't want to change it
	 * now - just need to get the batching working KT
	 */
	...
	/**
	 * This function returns true if there is at least one unclosed source
	...
	/**
	 * This function sends (best effort) shutdown messages to all source and
	 * sink streams
	 */
	...
	/**
	 * This function sends (best effort) shutdown messages to all source streams
	 * and eos/close messages to sink streams this will shutdown all source
	 * operators, but allow sink operators to complete processing as if stream
	...
	/**
	 * This function reads from a set of source streams specified by the
	 * variable activeSourceStreams and modifies sourceObject that contains the
	 * source element read and the stream from which it was read. If no source
	 * element was read, the the element field in sourceObject is set to null.
	 * This operation splits the responsiveness time of the operator among all
	 * the source streams it can read from. If this responsiveness time is
	 * exceeded, it returns null in the element field of sourceObject.
	 * 
	 * @exception java.lang.InterruptedException
	 *                Thrown if the thread is interrupted during execution write
	 *                to a previously closed stream
	...
	private void getFromSourceStreams(SourceStreamsObject sourceObject,
			boolean timedOutLastTime) throws InterruptedException,
			ShutdownException {
		...
		do {
			...
			Tuple tuple = sourceStreams[streamId].getTuple(timeout);
	 ...
	 * notifications instead of timeouts. Calling it will return a tuple or
	 * BLOCK until a notification is received. To avoid changing the rest of the
	 * code, once notified it returns null and the next call will have to pick
	 * up the actual tuple.
	 */
	...
	/**
	 * This function checks the sink streams for any control messages and
	 ...
	 * 
	 * @param ctrlFlag
	 *            The id of the control message received
	 * @param streamId
	 *            The id of the source stream from which the control message was
	 *            read
	 * 
	 * @exception java.lang.InterruptedException
	 ...
	 * 
	 * @exception java.lang.InterruptedException
	 *                Thrown if the thread is interrupted during execution
	 * @exception ShutdownException
	 *                query shutdown by user or execution error
	 */
	protected final void updatePartialResultCreation()
		...
		if (!activeSourceStreams.isEmpty()) {
			...
			for (int src = 0; src < numSourceStreams; ++src) {
				if (sourceStreams[src].getStatus() == SourceTupleStream.SynchPartial) {
					...
					activeSourceStreams.add(src);
				} else if (sourceStreams[src].getStatus() == SourceTupleStream.EndPartial) {
	 ...
	 *            the id of the control message received
	 * @param streamId
	 *            The id of the sink stream from which the control message was
	 *            read
	 * 
	 * @exception java.lang.InterruptedException
	 *                If the thread is interrupted during execution
	...
	/**
	 * This function processes a GetPartialResult control element received from
	 * a sink stream. If the control element is not a duplicate previously
	 * received from some other sink stream, then it is sent to all open source
	 * streams.
	 * 
	 * @param streamId
	 *            The id of the sink stream from which the getpartial element
	 *            was read
	 * 
	 * @exception java.lang.InterruptedException
	 *                If the thread is interrupted during execution
	...
	/**
	 * Puts a tuple into a specified sink stream. To be called by the operator
	 * implementations themselves. (sort of replaced putToDestinationStreams)
	 * 
	 * @param tuple
	 *            The tuple to be put in the stream
	 * @param streamId
	 *            the id of the stream where the tuple is to go
	 * 
	 * @exception java.lang.InterruptedException
	 *                Thread is interrupted during execution
	 * @exception ShutdownException
	...
	public final void putTuple(Tuple tuple, int streamId)
		...
		assert !sinkStreams[streamId].isClosed() : "KT putting tuple to closed stream - can I ignore this? previous code ignored it";
		...
		while (!sent) {
			...
			ArrayList ctrl = sinkStreams[streamId].putTuple(tuple); // XXX:
																	// perhaps
																	// out of
																	// the loop?
	...
	/**
	 * Display all attributes of a tuple, preceded by calling operator.  (JGM)
	 */
	public final void displayTuple(Tuple tuple, String message) {
		int attrNo;

		if (debug) {
			System.out.print(this.getName() + ": \"" + message + "\":  ");
			for (attrNo = 0; attrNo < tuple.size() - 1; attrNo++)
				System.out.print(tuple.getBaseAttribute(attrNo).toASCII() + ", ");
			System.out.println(tuple.getBaseAttribute(attrNo).toASCII());
		}
	}
	 ...
	 * Display all attributes of a tuple, preceded by calling operator.  (JGM)
	 */
	public final void displayTupleAlways(Tuple tuple, String message) {
		int attrNo;
	...
	/**
	 * Set the maximum amount of time a tuple can be delayed in the stream. In
	 * milliseconds.
	 */
	...
	/**
	 * This function is used by physical operators to send a message to a
	 * down-stream operator Currently, it is only used by PhysicalPunctQC to
	 * send msg (query parameters, EOS) to DBThread;
	 * 
	 * 
	 */
	protected void sendCtrlMsgUpStream(ControlFlag ctrlFlag, String ctrlMsg,
		...
		assert retCtrlFlag == ControlFlag.NULLFLAG : "Unexpected ctrl flag in sendCtrlMsgUpStream "
	 ...
	 * 
	 * @param ctrlFlag
	 *            The ide of the ctrl message to be sent
	 * @param streamId
	 *            The sink stream to which the control message is to be put
	 * 
	 * @exception ShutdownException
	 *                query shutdown by user or execution error
	 */
	private void sendCtrlMsgToSink(ControlFlag ctrlFlag, String ctrlMsg,
			int streamId) throws InterruptedException, ShutdownException {
		ArrayList newCtrl;
		do {
			newCtrl = sinkStreams[streamId].putCtrlMsg(ctrlFlag, ctrlMsg);
			if (newCtrl != null) {
	...
	/**
	 * This function puts a control element to all sink streams that have not
	...
	/**
	 * Compute the cost of constructing a tuple for the output schema of this
	 * operator
	 */
	protected double constructTupleCost(ICatalog catalog) {
		...
		return catalog.getDouble("tuple_construction_cost")
				+ catalog.getDouble("field_overhead")
				* getLogProp().getAttrs().size();
	}

	/**
	 * if an operator needs to do initialization, it should override this
	 * function
	 */
	 ...
	 * the operator is in a non-blocking state.
	 * 
	 * @param tupleElement
	...
	/**
	 * This function processes a tuple element read from a source stream when
	 * the operator is in a blocking state.
	 * 
	 * @param tupleElement
	 *            The tuple element read from a source stream
	 * @param streamId
	 ...
	 * 
	 * @exception ShutdownException
	 *                Shutdown due to execution error or client request
	 */
	...
	/**
	 * Requests that a (blocking) operator flush the current version of its
	 * results to the sink. This flush request may be do to a GET_PARTIAL
	 * request or due to end of stream.
	 * 
	 * @param partial
	 *            true if this is partial result output, false if this is the
	 *            final result output
	 * 
	 * @exception ShutdownException
	 *                query shutdown by user or execution error
	...
	/**
	 * This function handles punctuations for the given operator. The default
	 * behavior is to ignore the punctuation and continue
	 * 
	 * @param tuple
	...
	private void internalCleanUp(String msg) {
		if (niagara.connection_server.NiagraServer.TIME_OPERATORS) {
			cpuTimer.stop();
			cpuTimer.print(getName() + "(" + id + ")" + " (shutdown: " + msg
					+ ")");
		}
		if (isHeadOperator) {
	...
	/**
	 * <code>setResultDocument</code> provides an owner Document for XML nodes
	 * that are newly created by any operator in the query plan
	 * 
	 * @param doc
	...
	protected void dumpSchema(StringBuffer sb) {
		sb.append(" schema='").append(getLogProp().getAttrs().toString())
				.append("'");
	}
	...
	@Tunable(name = "instrumented", type = Tunable.TunableType.BOOLEAN, setter = "setInstrumented", description = "Enable/disable instrumentation")
	...
	@Tunable(name = "sampling", type = Tunable.TunableType.BOOLEAN, setter = "setSampling", description = "Enable/disable output tuple sampling")
	...
	@Tunable(name = "rateLimit", type = Tunable.TunableType.INTEGER, setter = "setRateLimit", description = "Maximum number of output tuples per collection period")
