 ...
 * A specialized version of Timer where static method parseTimeInterval() parses its input string as a long,
 * rather than as an int (ultimately for call from LongBucket).
 * 
 * Based off of Timer.
 */

public class LongTimer extends NullaryOperator {
	/**
	 * The epoch we're using to report time. Empty string means use the default
	 * Java timestamp (milliseconds after midnight, January 1, 1970), "now"
	 * means start from when the query plan started executing, anything else is
	...
	public void loadFromXML(Element e, LogicalProperty inputs[], Catalog catalog)
		...
		if (attrStr.length() == 0 || attrStr.equalsIgnoreCase("now"))
			...
			try {
				...
				throw new InvalidPlanException("Could not parse '" + attrStr
		...
		if (attrStr.length() == 0)
			throw new InvalidPlanException(
					"Period is a required attribute for timer");
	...
	/**
	 * @return the number of milliseconds in the specified time interval (if no
	 *         unit is provided, assume milliseconds)
	 */
	public static long parseTimeInterval(String intervalStr)
			throws InvalidPlanException {
		...
		while (strtok.hasMoreTokens()) {
			...
			if (expectNumber) {
				try {
					currentNumber = Long.parseLong(tok);
				} catch (NumberFormatException nfe) {
					throw new InvalidPlanException("Expected integer, found "
							+ tok + " while parsing " + intervalStr);
				}
			} else {
				tok = tok.toLowerCase();
				if (tok.indexOf("day") >= 0)
					...
					throw new InvalidPlanException("Expected time term, found "
							+ tok + " while parsing " + intervalStr);
			}
	...
	public void formatTimeInterval(long millis, StringBuffer sb) {
	...
	private void format(long value, int inMilliSecs, String unit, StringBuffer sb) {
	...
	public boolean equals(Object obj) {
		if (obj == null || !(obj instanceof LongTimer))
			return false;
		if (obj.getClass() != LongTimer.class)
			return obj.equals(this);
		LongTimer other = (LongTimer) obj;
		return name.equals(other.name) && relative.equals(other.relative)
				&& period == other.period && slack == other.slack
				&& granularity == other.granularity && warp == other.warp
				&& delay == other.delay;
	}
	 ...
	 */
	// (JGM) Unknown if this is a good hashcode - I had to cast result from long to int.
	public int hashCode() {
		return (int)(name.hashCode() ^ period ^ slack ^ granularity ^ warp ^ delay
				^ relative.hashCode());
	}
