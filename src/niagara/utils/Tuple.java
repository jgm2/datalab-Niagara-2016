...
public class Tuple {
    ...
    /**
     * (JGM) Used in displaying attribute values (not Object values).
     * 
     * This function returns a BaseAttr given an attribute's position
     *
     * @param position The position of the desired attribute
     *
     * @return The BaseAttr of the desired attribute
     */
    public BaseAttr getBaseAttribute(int position) {
        assert position
            < tupleSize : "Invalid position "
                + position
                + " tuple size is "
                + tupleSize;
        return (BaseAttr)tuple[position];
    }
