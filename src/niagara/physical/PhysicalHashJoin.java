	...
	protected void processTuple(Tuple tupleElement, int streamId)
		...
		if (!extensionJoin[streamId] || !producedResult) {
			...
			if (ts != null && ts[streamId] != null && ts[otherStreamId] != null) {
				//System.out.println("PhysicalHashJoin::streamId..." + streamId);  // (JGM)
				//System.out.println("left***" + ((BaseAttr)left.getAttribute(0)).toASCII());  // (JGM)
				//System.out.println("right***" + ((BaseAttr)right.getAttribute(0)).toASCII());  // (JGM)
...
//			for (int i = 0; i < 2; i++) {
//				System.out.println("PhysicalHashJoin::processPunctuation, partialSourceTuples[" + i + "].size():  " + partialSourceTuples[i].size());
//				System.out.println("PhysicalHashJoin::processPunctuation, finalSourceTuples[" + i + "].size():  " + finalSourceTuples[i].size());
//			}
//		}
//		tuplePrintCount++;
		...
		if (streamClose[otherStreamId]) {
			...
			//displayTupleAlways(tuple, "PhysicalHashJoin::processPunctuation first return");
			return;
		}
		...
		while (it.hasNext() && !fMatch) {
			...
			//System.out.println(otherStreamId);
			//displayTupleAlways(left, "left");
			//displayTupleAlways(right, "right");
		...
		//displayTupleAlways(tuple, "PhysicalHashJoin::processPunctuation end");
	}
	...
	private void passOnPunct(Punctuation tuple, int streamId)
		...
		//displayTupleAlways(tuple, "PhysicalHashJoin::PassOnPunct end");
	}
	...
	protected void producePunctuation(Punctuation left, Punctuation right,
		...
		//displayTupleAlways(result, "PhysicalHashJoin::producePunctuation end");
	...
	protected void opInitialize() {
		...
		//System.out.println(leftAttrs.get(0).getName());  // (JGM)
		//System.out.println(rightAttrs.get(0).getName());  // (JGM)
		...
		if (punctAttrs != null) {
			...
			for (int i = 0; i < punctAttrs.length; i++) {
				if (punctAttrs[i].compareTo("") != 0) {
					...
					//System.out.println(inputTupleSchemas[i].getVariableName(0));  // (JGM)
					//System.out.println(inputTupleSchemas[i].getVariableName(1));  // (JGM)
					//System.out.println(inputTupleSchemas[i].getVariableName(2));  // (JGM)
	...
	private void putPunctuation(Punctuation tuple, int streamId)
		...
		putTuple(result, 0);
		//displayTupleAlways(tuple, "PhysicalHashJoin::putPunctuation end");
	}
}
