...
import niagara.logical.LongBucket;
...
public class PhysicalLongBucket extends PhysicalOperator {
	...
	public void opInitFrom(LogicalOp logicalOperator) { // I think I should
		...
		propagate = ((LongBucket) logicalOperator).getPropagate();
		windowAttribute = ((LongBucket) logicalOperator).getWindowAttr();
		windowType = ((LongBucket) logicalOperator).getWindowType();
		range = ((LongBucket) logicalOperator).getWindowRange();
		slide = ((LongBucket) logicalOperator).getWindowSlide();
		widName = ((LongBucket) logicalOperator).getWid();
		start = ((LongBucket) logicalOperator).getStarttime();
		ae = new AtomicEvaluator(windowAttribute.getName());
		exploit = ((LongBucket) logicalOperator).getExploit();
		fAttr = ((LongBucket) logicalOperator).getFAttr();		
		
		logging = ((LongBucket) logicalOperator).getLogging();
		if (logging) {
	...
	public Op opCopy() {
		PhysicalLongBucket p = new PhysicalLongBucket();
	...
	protected void processTuple(Tuple inputTuple, int streamId)
		...
		//displayTuple(inputTuple, "PhysicalLongBucket::processTuple start");
		Tuple result;
		...
		count++;
		//System.out.println("In processTuple:  count = " + count);
...
//				}
	...
	protected Tuple appendWindowId(Tuple inputTuple, int streamId)
			throws ShutdownException, InterruptedException {
...
//				System.out.println("count = " + count);
	...
	protected void processPunctuation(Punctuation inputTuple, int streamId)
		...
		//displayTuple(inputTuple, "PhysicalLongBucket::processPunctuation start");
		//long wid = 0;
		long timestamp;
...
//		System.out.println("(timestamp - startSecond) % slide value:  " + Long.toString((timestamp - startSecond) % slide));
//		System.out.println("startSecond value:  " + Long.toString(startSecond));
		...
		displayTuple(inputTuple, "PhysicalLongBucket::processPunctuation end");
	...
	public boolean equals(Object o) {
		if (o == null || !(o instanceof PhysicalLongBucket))
			return false;
		if (o.getClass() != PhysicalLongBucket.class)
			return false;
		if(((PhysicalLongBucket)o).propagate != propagate)
			return false;
		if(((PhysicalLongBucket)o).exploit != exploit)
			return false;
		if(((PhysicalLongBucket)o).fAttr != fAttr)
			return false;
		
		PhysicalLongBucket another = (PhysicalLongBucket) o;
		...
		if (match)
			System.err.println("PhysicalLongBucket equal true");
		return match;
