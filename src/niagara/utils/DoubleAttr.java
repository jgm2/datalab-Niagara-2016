package niagara.utils;

import org.w3c.dom.Node;

public class DoubleAttr extends BaseAttr implements Arithmetics{
	 
	public static String name = "double";
	
	public DoubleAttr () {};
	
	public DoubleAttr (Node XMLNode) {
		loadFromXMLNode(XMLNode);
	}
	
	public DoubleAttr (Object value) {
		loadFromObject(value);
	}
	
	public void loadFromXMLNode(Node xmlNode) {
		short type = xmlNode.getNodeType();
		
		switch (type) {
		case Node.ELEMENT_NODE:
			attrVal = Double.valueOf(xmlNode.getFirstChild().getNodeValue());
			break;
		case Node.ATTRIBUTE_NODE:
			attrVal = Double.valueOf(xmlNode.getNodeValue());
			break;
		case Node.TEXT_NODE:
			attrVal = Double.valueOf(xmlNode.getNodeValue());
			break;
		default:
			throw new PEException("JL: Unsupported XML Node Type");
		}
	}

	public void loadPunctFromXMLNode(Node xmlNode) {
		short type = xmlNode.getNodeType();
		String nodeVal;
		
		switch (type) {
		case Node.ELEMENT_NODE:
			nodeVal = xmlNode.getFirstChild().getNodeValue();
			break;
		case Node.ATTRIBUTE_NODE:
			nodeVal = xmlNode.getNodeValue();
			break;
		case Node.TEXT_NODE:
			nodeVal = xmlNode.getNodeValue();
			break;
		default:
			nodeVal = null;
			throw new PEException("JL: Unsupported XML Node Type");
		}
		/*
		 * Right now, we only handle "*" for punctuation pattern
		 */
		if (nodeVal != null) {
			if (nodeVal.startsWith("*")) 
				attrVal = nodeVal;
			else
				attrVal = Double.valueOf(nodeVal);
		}
		punct = true;

	}
	
	public void loadFromObject(Object value) {
	
		if (value instanceof Long)
			attrVal = value;
		else if (value instanceof Integer)
			attrVal = value;
		else if (value instanceof Double)
			attrVal = value;
		else if (value instanceof String)
			attrVal = Double.parseDouble((String)value);
		else
			throw new PEException("JL: Unsupported Attribute Data Type");
	}
	
	public void loadFromValue(double value) {
		attrVal = new Double(value);
	}
	
	public String toASCII() {
		return attrVal.toString(); 
	}
	
	public boolean eq(BaseAttr other) {
		if (other.punct || punct) {
			throw new PEException("JL: Don't know how to do comparison with punctuation");
		}

		if (!(other instanceof DoubleAttr))
			return false;
		if (other.punct) {
			throw new PEException("JL: Don't know how to compare a double to a punctuation value");
		}
			
		return (((Double)attrVal).compareTo((Double)other.attrVal) == 0);
	}
	
	public boolean gt(BaseAttr other) {
		if (other.punct || punct) {
			throw new PEException("JL: Don't know how to do comparison with punctuation");
		}

		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");
		
		return (((Double)attrVal).compareTo((Double)other.attrVal) > 0);
	}

	public boolean lt(BaseAttr other) {
		if (other.punct || punct) {
			throw new PEException("JL: Don't know how to do comparison with punctuation");
		}

		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");

		return (((Double)attrVal).compareTo((Double)other.attrVal) < 0);
	}
	
	public BaseAttr copy() {
		return new DoubleAttr(attrVal);		
	}
	
	public DoubleAttr plus (BaseAttr other) {
		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");
		
		return new DoubleAttr((Double)attrVal + (Double)other.attrVal);
	}

	public DoubleAttr minus (BaseAttr other) {
		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");
		
		return new DoubleAttr((Double)attrVal - (Double)other.attrVal);
	}

	public DoubleAttr multiply (BaseAttr other) {
		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");
		
		return new DoubleAttr((Double)attrVal * (Double)other.attrVal);
	}

	public DoubleAttr divid (BaseAttr other) {
		if (!(other instanceof DoubleAttr))
			throw new PEException ("JL: Comparing to a different data type");
		
		return new DoubleAttr((Double)attrVal / (Double)other.attrVal);
	}

}